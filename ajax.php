...
/**
 * This file processes AJAX enrolment actions and returns JSON for the grabber enrolments plugin
 *
...
switch ($action) {
    ...
    case 'searchcohorts':
        ...
        $addedenrollment = optional_param('enrolcount', 0, PARAM_INT);
        $perpage = optional_param('perpage', 25, PARAM_INT);  //  This value is hard-coded to 25 in quickenrolment.js
        $outcome->response = enrol_grabber_get_potential_cohorts($context, $enrolid, $search, $page, $perpage, $addedenrollment);
        $outcome->success = true;
        break;
    case 'enrol':
        $enrolid = required_param('enrolid', PARAM_INT);
        ...
        if (empty($startdate)) {
            if (!$startdate = get_config('enrol_grabber', 'enrolstart')) {
